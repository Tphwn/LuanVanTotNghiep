...
export const suggestIconSlugFromName = (name) => {
  ...
  for (const { keys, slug } of NAME_KEYWORDS) {
    if (keys.some((k) => {
...
export const getAmenityIcon = (bieuTuong, ten = '') => {
  if (bieuTuong) {
    const key = String(bieuTuong).trim().toLowerCase();
    if (AMENITY_ICON_MAP[key]) return AMENITY_ICON_MAP[key];
    if (/^\p{Extended_Pictographic}/u.test(String(bieuTuong).trim())) return String(bieuTuong).trim();
  }
  if (ten) {
    const slug = suggestIconSlugFromName(ten);
    if (AMENITY_ICON_MAP[slug]) return AMENITY_ICON_MAP[slug];
  }
  return '🛎️';
};

export const resolveIconSlug = (bieuTuong, ten = '') => {
  if (bieuTuong && AMENITY_ICON_MAP[String(bieuTuong).toLowerCase()]) {
    return String(bieuTuong).toLowerCase();
  }
  if (bieuTuong && /^\p{Extended_Pictographic}/u.test(String(bieuTuong).trim())) {
    return String(bieuTuong).trim();
  }
