      ...
      <div className="modal-box" style={{ maxWidth:620 }} onClick={e=>e.stopPropagation()}>
        ...
        {tx.hoan_tien && (
          <div style={{ marginTop:14, padding:12, background:'#fff8e6', borderRadius:8, border:'1px solid #fac775', fontSize:13 }}>
            <strong>↩️ Hoàn tiền:</strong> {fmt(tx.hoan_tien.so_tien_hoan)} — {REFUND_STATUS[tx.hoan_tien.trang_thai]?.label}
          </div>
        )}
      </div>
    </div>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" style={{ maxWidth:460 }} onClick={e=>e.stopPropagation()}>
        ...
        <textarea rows={3} value={lyDo} onChange={e=>setLyDo(e.target.value)}
          ...
          style={{ width:'100%', padding:'9px 12px', border:'1px solid #d4ede6', borderRadius:8, fontSize:14, resize:'vertical', fontFamily:'inherit', boxSizing:'border-box' }}
        />
...
const AdminPaymentsPage = () => {
  ...
  const tabs = [
    ...
    { id:'refunds',     label:`↩️ Hoàn tiền${refunds.filter(r=>r.trang_thai==='cho_xu_ly').length > 0 ? ` (${refunds.filter(r=>r.trang_thai==='cho_xu_ly').length})` : ''}` },
        ...
        <>
          ...
          {pendingRefunds > 0 && (
            <div style={{
              ...
              <span>⚠️ Có <strong>{pendingRefunds}</strong> yêu cầu hoàn tiền đang chờ xử lý</span>
        ...
        <>
          ...
          <div className="content-card">
            ...
            {loading ? (
              ...
              <div className="empty-state"><div className="empty-state-icon">💳</div><p className="empty-state-text">Chưa có giao dịch</p></div>
            ) : (
      ...
      {tab === 'refunds' && (
        <>
          ...
          <div className="content-card">
            ...
            {refunds.length === 0 ? (
              <div className="empty-state"><div className="empty-state-icon">↩️</div><p className="empty-state-text">Chưa có yêu cầu hoàn tiền</p></div>
            ) : (
              <table className="data-table">
                <thead>
                  <tr>
        ...
        <>
          ...
          <div className="content-card" style={{ marginBottom:16 }}>
            ...
            {commByPartner.length === 0 ? (
              <div className="empty-state"><div className="empty-state-icon">💰</div><p className="empty-state-text">Chưa có dữ liệu</p></div>
            ) : (
              <table className="data-table">
                <thead>
                  <tr>
          ...
          <div className="content-card">
            ...
            {commissions.length === 0 ? (
              <div className="empty-state"><div className="empty-state-icon">💰</div><p className="empty-state-text">Chưa có hoa hồng</p></div>
            ) : (
              <table className="data-table">
                <thead>
                  <tr>
